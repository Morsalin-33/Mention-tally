...
import { Pencil, Check, X, Trash2, RotateCcw, Plus, Crown, Minimize2, Maximize2, ShieldCheck, History } from "lucide-react";

const STORAGE_KEY = "mention-tally-v2";
const BACKUP_KEY = "mention-tally-v2-backup";
...
export default function MentionTally() {
  ...
  const [backdateCount, setBackdateCount] = useState("");
  const [lastBackdateCount, setLastBackdateCount] = useState(null);
  const [restoreOpen, setRestoreOpen] = useState(false);
  const [backupPreview, setBackupPreview] = useState(undefined); // undefined = not checked yet, null = none exists
  const [restoreDone, setRestoreDone] = useState(false);
  const peopleRef = useRef(people);
  const stateRef = useRef({ activeId, compact });


  useEffect(() => {
    peopleRef.current = people;
    stateRef.current = { activeId, compact };
  }, [people, activeId, compact]);

  async function maybeAutoBackup() {
    try {
      let last = null;
      try {
        const res = await window.storage.get(BACKUP_KEY, false);
        if (res && res.value) last = JSON.parse(res.value);
      } catch (e) {
        // no backup saved yet
      }
      const now = Date.now();
      const dueForBackup = !last || now - new Date(last.backedUpAt).getTime() >= SIX_HOURS_MS;
      if (dueForBackup && peopleRef.current.length > 0) {
        await window.storage.set(
          BACKUP_KEY,
          JSON.stringify({
            people: peopleRef.current,
            activeId: stateRef.current.activeId,
            compact: stateRef.current.compact,
  ...
  useEffect(() => {
    ...
    // Catch up on backups every 30 min while the app is open, in case it's left open for hours.
    const interval = setInterval(() => maybeAutoBackup(), 30 * 60 * 1000);
  ...
  function openBackdate() {
    // Default to yesterday, and remember the last count you typed in.
    const y = new Date();
    y.setDate(y.getDate() - 1);
    setBackdateDate(localDayKey(y));
    setBackdateCount(lastBackdateCount != null ? String(lastBackdateCount) : "");
    setBackdateOpen(true);
  }

  function handleBackdateSubmit() {
    const count = parseInt(backdateCount, 10);
    if (!backdateDate || !count || count <= 0) return;
    const [y, m, d] = backdateDate.split("-").map(Number);
    // Spread entries across the day in ~6-hour blocks (6am, 12pm, 6pm, 10pm) so they don't clump.
    const slots = [6, 12, 18, 22];
    const mentions = [];
    for (let i = 0; i < count; i++) {
      const slotHour = slots[i % slots.length];
      const minute = Math.floor(Math.random() * 60);
      const t = new Date(y, m - 1, d, slotHour, minute, Math.floor(Math.random() * 60));
      mentions.push(t.toISOString());
    }
    updateActiveMentions((existing) => [...existing, ...mentions].sort());
    setLastBackdateCount(count);
  ...
  async function openRestore() {
    ...
    setBackupPreview(undefined); // "checking..." state
    setRestoreOpen(true);
    try {
      const res = await window.storage.get(BACKUP_KEY, false);
      if (res && res.value) {
        setBackupPreview(JSON.parse(res.value));
      } else {
        setBackupPreview(null); // no backup exists yet
      }
    } catch (e) {
  ...
  function handleRestoreConfirm() {
    if (!backupPreview || !Array.isArray(backupPreview.people)) return;
    const restoredPeople = backupPreview.people;
    const restoredActiveId = backupPreview.activeId || (restoredPeople[0] && restoredPeople[0].id) || null;
    setPeople(restoredPeople);
    setActiveId(restoredActiveId);
    if (typeof backupPreview.compact === "boolean") setCompact(backupPreview.compact);
    persist(restoredPeople, restoredActiveId, backupPreview.compact);
    setRestoreDone(true);
  }
  ...
  const backdateModal = backdateOpen && (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-6" style={{ backgroundColor: "rgba(20,18,16,0.5)" }}>
      <div className="w-full max-w-xs rounded-2xl p-5" style={{ backgroundColor: COLORS.cardLight }}>
        <div className="text-sm font-semibold mb-3" style={{ color: COLORS.graphite }}>
          Log for {activePerson.name} — earlier day
        </div>
        <label className="block text-xs mb-1" style={{ color: COLORS.inkMuted }}>Date</label>
        <input
          type="date"
          value={backdateDate}
          onChange={(e) => setBackdateDate(e.target.value)}
          max={localDayKey(new Date())}
          className="mt-focus w-full mb-3 px-3 py-2 rounded-lg text-sm"
          style={{ backgroundColor: COLORS.bone, color: COLORS.graphite, border: "none" }}
        />
        <label className="block text-xs mb-1" style={{ color: COLORS.inkMuted }}>
          How many mentions that day?
        </label>
        <input
          type="number"
          inputMode="numeric"
          min="1"
          value={backdateCount}
          onChange={(e) => setBackdateCount(e.target.value)}
          placeholder={lastBackdateCount != null ? String(lastBackdateCount) : "e.g. 10"}
          className="mt-focus w-full mb-1 px-3 py-2 rounded-lg text-sm"
          style={{ backgroundColor: COLORS.bone, color: COLORS.graphite, border: "none" }}
        />
        <div className="text-xs mb-4" style={{ color: COLORS.inkMuted }}>
          Spread across ~6-hour blocks (6am / noon / 6pm / 10pm) that day.
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => setBackdateOpen(false)}
            className="mt-focus flex-1 py-2 rounded-lg text-sm font-medium"
          ...
          <button
            ...
            className="mt-focus flex-1 py-2 rounded-lg text-sm font-semibold text-white"
            style={{ backgroundColor: activeColor.ink }}
          >
            Add
          </button>
        </div>
  ...
  const restoreModal = restoreOpen && (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-6" style={{ backgroundColor: "rgba(20,18,16,0.5)" }}>
      <div className="w-full max-w-xs rounded-2xl p-5" style={{ backgroundColor: COLORS.cardLight }}>
        <div className="flex items-center gap-2 text-sm font-semibold mb-3" style={{ color: COLORS.graphite }}>
          <History size={16} /> Restore from backup
        </div>

        {backupPreview === undefined && (
          <div className="text-xs mb-4" style={{ color: COLORS.inkMuted }}>Checking for a backup…</div>
        )}

        {backupPreview === null && (
          <div className="text-xs mb-4" style={{ color: COLORS.inkMuted }}>
            No backup saved yet. One is taken automatically every 6 hours while you use the app — check back later.
          </div>
        )}

        {backupPreview && restoreDone && (
          <div className="text-xs mb-4 flex items-center gap-1.5" style={{ color: COLORS.inkMuted }}>
            <ShieldCheck size={14} /> Restored. Your current data now matches this backup.
          </div>
        )}
        ...
        {backupPreview && !restoreDone && (
          <>
            <div className="text-xs mb-3" style={{ color: COLORS.inkMuted }}>
              Last saved automatically: {new Date(backupPreview.backedUpAt).toLocaleString()}
            </div>
            <div className="rounded-lg p-3 mb-4 text-xs space-y-1" style={{ backgroundColor: COLORS.bone, color: COLORS.graphite }}>
              {(backupPreview.people || []).map((p) => (
                <div key={p.id} className="flex justify-between">
                  <span>{p.name}</span>
                  <span className="font-semibold">{p.mentions.length}</span>
                </div>
              ))}
            </div>
            <div className="text-xs mb-4" style={{ color: COLORS.inkMuted }}>
              Restoring replaces what's currently in the app with this backup.
            </div>
          </>
        )}
    ...
    return (
      <div className="mt-body min-h-screen relative flex flex-col items-center justify-center px-6 py-8" style={{ backgroundColor: COLORS.paper }}>
        ...
        <div className="w-full max-w-xs text-center">
          ...
          <button onClick={openBackdate} className="mt-focus flex items-center justify-center gap-1.5 text-xs mt-1 py-1 mx-auto underline" style={{ color: COLORS.inkMuted }}>
            Log for an earlier day
          </button>
          <button onClick={openRestore} className="mt-focus flex items-center justify-center gap-1.5 text-xs py-1 mx-auto underline" style={{ color: COLORS.inkMuted }}>
            Restore backup
          </button>

          {confirmDeletePerson ? (
            <div className="flex items-center justify-center gap-3 text-xs mt-3">
              <span style={{ color: COLORS.inkMuted }}>Remove {activePerson.name}?</span>
              <button onClick={handleDeletePerson} className="mt-focus font-semibold" style={{ color: "#A24936" }}>Yes</button>
              <button onClick={() => setConfirmDeletePerson(false)} className="mt-focus" style={{ color: COLORS.inkMuted }}>Cancel</button>
            </div>
          ) : (
            <button onClick={() => setConfirmDeletePerson(true)} className="mt-focus flex items-center justify-center gap-1.5 text-xs mt-3 py-1 mx-auto" style={{ color: COLORS.inkMuted }}>
              Remove person
            </button>
          )}
  ...
  return (
    <div className="mt-body min-h-screen relative flex flex-col items-center px-4 py-8" style={{ backgroundColor: COLORS.paper }}>
      ...
      <div className="w-full max-w-sm">
        ...
        <button onClick={openBackdate} className="mt-focus w-full flex items-center justify-center gap-1.5 text-xs py-1 underline" style={{ color: COLORS.inkMuted }}>
          Log for an earlier day
        </button>
        <button onClick={openRestore} className="mt-focus w-full flex items-center justify-center gap-1.5 text-xs py-1 underline" style={{ color: COLORS.inkMuted }}>
          Restore backup
        </button>
